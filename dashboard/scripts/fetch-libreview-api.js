#!/usr/bin/env node

/**
 * Fetch glucose readings from LibreView API (unofficial)
 * Usage: node scripts/fetch-libreview-api.js [--days=14] [--import]
 */

const https = require('https');
require('dotenv').config();

const LIBREVIEW_API = 'https://api-eu2.libreview.io';
const DAYS_DEFAULT = 14;

// Parse args
const args = process.argv.slice(2);
const daysArg = args.find(a => a.startsWith('--days='));
const shouldImport = args.includes('--import');
const daysToFetch = daysArg ? parseInt(daysArg.split('=')[1]) : DAYS_DEFAULT;

async function httpsRequest(options, body = null) {
  return new Promise((resolve, reject) => {
    const req = https.request(options, res => {
      let data = '';
      res.on('data', chunk => data += chunk);
      res.on('end', () => {
        try {
          const parsed = JSON.parse(data);
          if (res.statusCode >= 400) {
            reject(new Error(`HTTP ${res.statusCode}: ${parsed.message || data}`));
          } else {
            resolve(parsed);
          }
        } catch (e) {
          reject(new Error(`Failed to parse response: ${data}`));
        }
      });
    });
    req.on('error', reject);
    if (body) {
      req.write(JSON.stringify(body));
    }
    req.end();
  });
}

async function login() {
  const email = process.env.LIBREVIEW_EMAIL;
  const password = process.env.LIBREVIEW_PASSWORD;

  if (!email || !password) {
    throw new Error('LIBREVIEW_EMAIL and LIBREVIEW_PASSWORD must be set in .env');
  }

  console.error('🔐 Logging in to LibreView...');

  const response = await httpsRequest({
    hostname: 'libreview-proxy.onrender.com',
    path: '/us/llu/auth/login',
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
      'User-Agent': 'Mozilla/5.0',
      'product': 'llu.android',
      'version': '4.16.0'
    }
  }, {
    email,
    password
  });

  if (!response.data || !response.data.authTicket || !response.data.authTicket.token) {
    console.error('Login response:', JSON.stringify(response, null, 2));
    throw new Error('Login failed: no token in response');
  }

  console.error('✅ Login successful');
  return response.data.authTicket.token;
}

async function getConnections(token) {
  console.error('📋 Fetching connections...');

  const response = await httpsRequest({
    hostname: 'libreview-proxy.onrender.com',
    path: '/us/llu/connections',
    method: 'GET',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Accept': 'application/json',
      'User-Agent': 'Mozilla/5.0',
      'product': 'llu.android',
      'version': '4.16.0'
    }
  });

  if (!response.data || !Array.isArray(response.data)) {
    throw new Error('No connections found');
  }

  console.error(`✅ Found ${response.data.length} connection(s)`);
  return response.data;
}

async function getGraphData(token, patientId) {
  console.error(`📊 Fetching glucose graph data for patient ${patientId}...`);

  const response = await httpsRequest({
    hostname: 'libreview-proxy.onrender.com',
    path: `/us/llu/connections/${patientId}/graph`,
    method: 'GET',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Accept': 'application/json',
      'User-Agent': 'Mozilla/5.0',
      'product': 'llu.android',
      'version': '4.16.0'
    }
  });

  if (!response.data || !response.data.graphData) {
    throw new Error('No graph data in response');
  }

  console.error(`✅ Fetched graph data`);
  return response.data;
}

function convertToCSV(graphData, patientName, offset = 0) {
  const readings = graphData.graphData || [];
  
  // LibreView API returns timestamps in milliseconds
  const now = Date.now();
  const cutoff = now - (daysToFetch * 24 * 60 * 60 * 1000);

  const filtered = readings.filter(r => {
    const ts = new Date(r.Timestamp).getTime();
    return ts >= cutoff;
  });

  console.error(`📝 Converting ${filtered.length} readings to CSV (last ${daysToFetch} days)...`);

  const rows = filtered.map(reading => {
    const ts = new Date(reading.Timestamp);
    const value = parseFloat(reading.Value);
    const adjusted = offset ? (value + offset).toFixed(1) : value.toFixed(1);
    
    // CSV format matching LibreView export:
    // Device,Serial Number,Device Timestamp,Record Type,Historic Glucose mmol/L,...
    return [
      'FreeStyle Libre',
      '', // serial
      ts.toISOString().replace('T', ' ').replace('Z', ''),
      '0', // record type (0 = historic glucose)
      adjusted,
      '', '', '', '', '', '' // empty columns for other fields
    ].join(',');
  });

  const header = 'Device,Serial Number,Device Timestamp,Record Type,Historic Glucose mmol/L,Scan Glucose mmol/L,Non-numeric Rapid-Acting Insulin,Rapid-Acting Insulin (units),Non-numeric Food,Carbohydrates (grams),Carbohydrates (servings),Non-numeric Long-Acting Insulin,Long-Acting Insulin (units),Notes,Strip Glucose mmol/L,Ketone mmol/L,Meal Insulin (units),Correction Insulin (units),User Change Insulin (units)';

  return [header, ...rows].join('\n');
}

async function importCSV(csvData) {
  const { execSync } = require('child_process');
  const fs = require('fs');
  const path = require('path');
  
  const tmpFile = path.join(__dirname, `libreview-api-${Date.now()}.csv`);
  fs.writeFileSync(tmpFile, csvData);
  
  console.error(`📥 Importing to database...`);
  
  try {
    execSync(`node ${path.join(__dirname, 'import-glucose.js')} ${tmpFile}`, {
      stdio: 'inherit'
    });
    fs.unlinkSync(tmpFile);
    console.error('✅ Import complete, temp file cleaned up');
  } catch (e) {
    console.error('❌ Import failed:', e.message);
    console.error(`Temp CSV saved at: ${tmpFile}`);
    process.exit(1);
  }
}

async function main() {
  try {
    const token = await login();
    const connections = await getConnections(token);
    
    // Use first connection (Archer)
    const archer = connections[0];
    if (!archer) {
      throw new Error('No connections found');
    }
    
    console.error(`👤 Using connection: ${archer.firstName} ${archer.lastName}`);
    
    const graphData = await getGraphData(token, archer.patientId);
    
    const offset = parseFloat(process.env.LIBRE_OFFSET || 0);
    const csv = convertToCSV(graphData, archer.firstName, offset);
    
    if (shouldImport) {
      await importCSV(csv);
    } else {
      console.log(csv);
    }
  } catch (error) {
    console.error('❌ Error:', error.message);
    process.exit(1);
  }
}

main();
